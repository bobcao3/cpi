...
let destroyApp: () => void = () => {};
export function setDestroyApp(fn: () => void): void {
  destroyApp = fn;
}
...
export function goto(v: View): void {
  setView(v);
}
...
// --- project list ---
export function navProjectDown(): void {
  setSelProject((s) => clamp(s + 1, 0, Math.max(0, listProjects().length - 1)));
}
export function navProjectUp(): void {
  setSelProject((s) => clamp(s - 1, 0, Math.max(0, listProjects().length - 1)));
}
export function openSelectedProject(): void {
  const ps = listProjects();
  if (ps.length === 0) {
    openPrompt("New project", "", newProject);
    return;
  }
...
export function newProject(name: string): void {
  try {
    const p: ProjectRow = createProject(name, null);
    bump();
    flash("ok", `Created project ${name}`);
    openProject(p.id);
  } catch (e) {
    flash("error", errMsg(e));
  }
...
export function navDown(): void {
  ...
  if (selTask() + 1 < tasks.length) {
    setSelTask(selTask() + 1);
    return;
  }
  setSelCol((ci + 1) % cols.length);
  setSelTask(0);
}
export function navUp(): void {
  ...
  setSelCol(prev);
  setSelTask(Math.max(0, tasks.length - 1));
}
export function navLeft(): void {
  ...
  const cols = listColumns(pid);
  if (cols.length === 0) return;
  setSelCol((c) => (c - 1 + cols.length) % cols.length);
  setSelTask(0);
}
export function navRight(): void {
  ...
  const cols = listColumns(pid);
  if (cols.length === 0) return;
  setSelCol((c) => (c + 1) % cols.length);
  setSelTask(0);
}

export function openCurrentCard(): void {
  const c = curTask();
  if (c) openCard(c.task.id);
}
...
export function newCard(title: string): void {
  ...
  if (!col) {
    flash(
      "error",
      "no column — add one first (clidos -p <project> columns create)",
    );
    return;
  }
...
function moveCardSel(dir: -1 | 1): void {
  ...
  const ci = clamp(selCol(), 0, cols.length - 1);
  const dst = ci + dir;
  if (dst < 0 || dst >= cols.length) {
    flash(
      "error",
      dir < 0 ? "already at the first column" : "already at the last column",
    );
    return;
  }
  ...
  setSelCol(dst);
  setSelTask(Math.max(0, listTasks(c.p, target.id).length - 1));
}
export function moveCardLeft(): void {
  moveCardSel(-1);
}
export function moveCardRight(): void {
  moveCardSel(1);
}
...
export function toggleDone(): void {
  ...
  write(done ? "Reopened" : "Completed", () =>
    setTaskCompleted(c.p, c.task.id, !done),
  );
}
...
export function archiveCurrentCard(): void {
  const c = curTask();
  if (!c) return;
  const col = listColumns(c.p)[
    clamp(selCol(), 0, listColumns(c.p).length - 1)
  ]!;
  try {
  ...
  setSelTask(
    clamp(selTask(), 0, Math.max(0, listTasks(c.p, col?.id ?? "").length - 1)),
  );
}
export function unarchiveLastArchive(): void {
  const a = lastArchive;
  if (!a) {
    flash("warn", "nothing to restore");
    return;
  }
  if (a.projectId !== projectId()) {
    flash("warn", "nothing to restore in this project");
    return;
  }
  try {
    ...
    flash(
      "ok",
      relocated
        ? `Restored ${title} — moved to ${column}`
        : `Restored ${title}`,
    );
  } catch (e) {
    flash("error", errMsg(e));
...
export function editCardTitle(): void {
  ...
  openPrompt("Edit card title", c.task.title, (v) => {
    const pid = projectId();
    if (pid) write("Updated", () => updateTask(pid, c.task.id, { title: v }));
  });
}
...
export function toggleDoneCard(): void {
  ...
  const cid = cardId();
  if (!pid || !cid) return;
  const t = getTask(pid, cid);
  if (!t) return;
  const done = t.completed_at != null;
  write(done ? "Reopened" : "Completed", () =>
    setTaskCompleted(pid, cid, !done),
  );
}
...
export function toggleTopicOnCard(topicId: string): void {
  ...
  write(on ? "Untagged" : "Tagged", () =>
    on ? detachTopic(pid, cid, topicId) : attachTopic(pid, cid, topicId),
  );
}
...
export function toggleCurrentTopicOnCard(): void {
  const pid = projectId();
  if (!pid) return;
  const ts = listTopics(pid);
  const t = ts[clamp(selTopic(), 0, Math.max(0, ts.length - 1))];
  if (!t) return;
  toggleTopicOnCard(t.id);
}

// --- topics view ---
export function navTopicDown(): void {
  const pid = projectId();
  if (!pid) return;
  setSelTopic((s) => clamp(s + 1, 0, Math.max(0, listTopics(pid).length - 1)));
}
export function navTopicUp(): void {
  const pid = projectId();
  if (!pid) return;
  setSelTopic((s) => clamp(s - 1, 0, Math.max(0, listTopics(pid).length - 1)));
}
...
export function renameCurrentTopic(): void {
  ...
  if (!pid) return;
  const ts = listTopics(pid);
  const t = ts[clamp(selTopic(), 0, ts.length - 1)];
  if (!t) return;
  openPrompt("Rename topic", t.name, (v) => {
    if (pid) write("Renamed", () => renameTopic(pid, t.id, v));
  });
}
