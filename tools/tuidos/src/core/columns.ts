...
function nextColumnPosition(db: Database): number {
  const row = db.prepare("SELECT COALESCE(MAX(position), -1) + 1 AS p FROM column_display").get() as { p: number };
  return row.p;
}
...
export function createColumn(projectId: string, name: string): ColumnRow {
  ...
  try {
    db.transaction(() => {
      ...
      db.prepare("INSERT INTO column_display (column_id, position) VALUES (?, ?)").all(id, position);
      recordAudit(db, {
        ts: now, project_id: projectId, action: "column.create", entity_type: "column",
        entity_id: id, summary: cap(`created column ${name}`),
      });
    })();
...
export function renameColumn(projectId: string, columnId: string, newName: string): string {
  ...
  try {
    db.transaction(() => {
      const row = db.prepare(`SELECT name FROM columns WHERE id = ? AND archived_at IS NULL`).get(columnId) as
        | { name: string } | null;
      if (!row) throw new Error(`no active column '${columnId}'`);
      oldName = row.name;
      db.prepare("UPDATE columns SET name = ?, updated_at = ? WHERE id = ?").all(newName, now, columnId);
      recordAudit(db, {
        ts: now, project_id: projectId, action: "column.rename", entity_type: "column",
        entity_id: columnId, summary: cap(`renamed column ${oldName} → ${newName}`),
      });
    })();
  } catch (e) {
    if (isUniqueViolation(e))
      throw new Error(`a column named '${newName}' already exists — use a different name`);
    throw e;
  } finally {
    db.close();
  }
...
export function moveColumn(projectId: string, columnId: string, newPosition: number): void {
  const now = Date.now();
  const db = openProjectWrite(projectId);
  try {
    db.transaction(() => {
      ...
      if (!Number.isInteger(newPosition) || newPosition < 0 || newPosition >= cols.length)
        throw new Error(`position must be 0..${cols.length - 1} (got ${newPosition})`);
      ...
      recordAudit(db, {
        ts: now, project_id: projectId, action: "column.move", entity_type: "column",
        entity_id: columnId, summary: cap(`reordered column ${moved!.name} to position ${newPosition}`),
      });
    })();
  } finally {
    db.close();
...
export function archiveColumn(projectId: string, columnId: string): string {
  ...
  try {
    db.transaction(() => {
      const row = db.prepare(`SELECT name, archived_at FROM columns WHERE id = ?`).get(columnId) as
        | { name: string; archived_at: number | null } | null;
      if (!row) throw new Error(`no column '${columnId}'`);
      if (row.archived_at != null) throw new Error(`column '${row.name}' is already archived`);
      name = row.name;
      const { n } = db.prepare(
      ...
      db.prepare("UPDATE columns SET archived_at = ?, updated_at = ? WHERE id = ?").all(now, now, columnId);
      recordAudit(db, {
        ts: now, project_id: projectId, action: "column.archive", entity_type: "column",
        entity_id: columnId, summary: cap(`archived column ${name}`),
      });
    })();
  } finally {
    db.close();
