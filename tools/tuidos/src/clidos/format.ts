...
export function renderProjectList(projects: ProjectRow[]): string {
  ...
  return projects
    .map((p) => {
      const line = `  ${padAccent(p.name, width)}  ${uid(p.id, projects.map((x) => x.id))}  ${muted(relativeTime(p.updated_at))}`;
...
export function renderProjectBullets(projects: ProjectRow[]): string {
  if (projects.length === 0) return muted("(no projects)");
  return projects
    .map((p) => `${muted("--")} ${accent(p.name)} ${uid(p.id, projects.map((x) => x.id))} ${muted(relativeTime(p.updated_at))}`)
    .join("\n");
}
...
export function renderTopicList(topics: TopicRow[]): string {
  ...
  return topics
    .map((t) => `  ${padAccent(t.name, width)}  ${uid(t.id, topics.map((x) => x.id))}  ${muted(relativeTime(t.updated_at))}`)
    .join("\n");
}
...
export function renderDiscovery(v: DiscoveryView): string {
  const pad = Math.max("project".length, "audit".length, "topics".length, "task".length, "columns".length);
  const lines: string[] = [];
  ...
  lines.push(`  ${padAccent("task", pad)}  manage a project's cards (-p <id-or-name>)`);
  lines.push(`  ${padAccent("columns", pad)}  manage a project's columns (-p <id-or-name>)`);
...
/** ✓ Created project <name> (<id>) — name accented, id shown plainly. */
export function renderProjectCreated(name: string, id: string): string {
  return `${ok("✓")} Created project ${accent(name)} (${id})`;
}

/** ✓ Added N columns, M topics — or a note if defaults already present. */
export function renderProjectDefaultsApplied(result: { columns: number; topics: number }): string {
  const { columns, topics } = result;
  if (columns === 0 && topics === 0) return note("defaults already present (no changes)");
  const parts: string[] = [];
