...
export const messageSubcommand = defineCommand({
  ...
  subCommands: {
    add: defineCommand({
      meta: {
        name: "add",
        description: "Append a message (the first is the body)",
      },
      args: {
        task: {
          type: "positional",
        ...
        content: {
          type: "positional",
          description: "Message text (quote multi-word)",
          required: true,
        },
        author: {
          type: "string",
          description:
            "Override the author (default: your VCS user.name <user.email>)",
        },
      },
      run({ args }) {
        ...
        if (!args.author) {
          ...
          if (id.source === "fallback")
            warn(fallbackWarning(id), fallbackRemedy());
        }
        const m = guard(() =>
          createMessage(projectId, taskId, author, args.content),
        );
    ...
    edit: defineCommand({
      ...
      run({ args }) {
        if (!args.task || !args.message || !args.content)
          fail("edit requires <task> <message> <content>");
        const projectId = requireProject();
        const taskId = resolveTaskId(projectId, args.task);
    ...
    archive: defineCommand({
      ...
      run({ args }) {
        if (!args.task || !args.message)
          fail("archive requires <task> <message>");
        const projectId = requireProject();
        const taskId = resolveTaskId(projectId, args.task);
        const messageId = resolveMessageId(projectId, taskId, args.message);
