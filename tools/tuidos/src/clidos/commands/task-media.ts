...
import {
  resolveTaskId, resolveMessageId, resolveMediaId,
  renderMediaAdded, renderMediaRemoved, renderMediaList, renderMediaExported,
} from "../card-view";

export const mediaSubcommand = defineCommand({
  meta: { name: "media", description: "Manage media attached to a task's messages" },
  subCommands: {
    add: defineCommand({
      ...
      args: {
        ...
        file: { type: "positional", description: "Path to the file", required: true },
        filename: { type: "string", description: "Override stored filename" },
      },
      run({ args }) {
        if (!args.task || !args.message || !args.file) fail("add requires <task> <message> <file>");
        const projectId = requireProject();
        const taskId = resolveTaskId(projectId, args.task);
        const messageId = resolveMessageId(projectId, taskId, args.message);
        const m = guard(() => addMedia(projectId, taskId, messageId, args.file, args.filename ?? null, null));
        const ids = listMediaForTask(projectId, taskId).map((x) => x.id);
        console.log(renderMediaAdded(m.filename, uid(m.id, ids)));
      },
    }),
    list: defineCommand({
      meta: { name: "list", description: "List media across a task's messages" },
    ...
    remove: defineCommand({
      ...
      args: {
        task: { type: "positional", description: "Task id or prefix", required: true },
        media: { type: "positional", description: "Media id or prefix", required: true },
      },
      run({ args }) {
    ...
    export: defineCommand({
      meta: { name: "export", description: "Copy a media blob to a destination path" },
      args: {
        task: { type: "positional", description: "Task id or prefix", required: true },
        media: { type: "positional", description: "Media id or prefix", required: true },
      ...
      run({ args }) {
        if (!args.task || !args.media || !args.dest) fail("export requires <task> <media> <dest>");
        const projectId = requireProject();
        const taskId = resolveTaskId(projectId, args.task);
        const mediaId = resolveMediaId(projectId, taskId, args.media);
        const media = listMediaForTask(projectId, taskId).find((m) => m.id === mediaId);
