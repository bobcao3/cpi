...
export const columnsCommand = defineCommand({
  meta: {
    name: "columns",
    description: "Manage a project's columns (use -p <id-or-name>)",
  },
  subCommands: {
    list: defineCommand({
    ...
    create: defineCommand({
      meta: {
        name: "create",
        description: "Create a column (appended to the end)",
      },
      args: {
        name: {
          type: "positional",
    ...
    rename: defineCommand({
      ...
      run({ args }) {
        if (!args.column || !args.name)
          fail("rename requires <column> <new-name>");
        const projectId = requireProject();
        const columnId = resolveColumnId(projectId, args.column);
        const oldName = guard(() =>
          renameColumn(projectId, columnId, args.name),
        );
    ...
    move: defineCommand({
      meta: {
        name: "move",
        description: "Reorder a column to a 0-based position",
      },
      args: {
        column: {
          type: "positional",
        ...
        position: {
          type: "positional",
          description: "0-based position",
          required: true,
        },
      },
      run({ args }) {
        if (!args.column || args.position == null)
          fail("move requires <column> <position>");
        const n = Number(args.position);
        if (!Number.isInteger(n) || n < 0)
          fail(
            `invalid position: ${args.position} (must be a non-negative integer)`,
          );
        const projectId = requireProject();
        const columnId = resolveColumnId(projectId, args.column);
    ...
    archive: defineCommand({
      meta: {
        name: "archive",
        description: "Archive a column (move its tasks first)",
      },
      args: {
        column: {
          type: "positional",
  ...
  async run({ rawArgs }) {
    ...
    console.log(
      renderColumnList(listColumns(projectId), countTasksByColumn(projectId)),
    );
  },
});
