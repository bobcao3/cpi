...
try {
  ...
  if (record) {
    ...
    while (exited === null && Date.now() < exitDl)
      await new Promise((res) => setTimeout(res, 50));
    ok(exited === 0, "resume client saw exit 0 (got " + exited + ")");
    ok(
      /r[1-8]/.test(saw),
      "resume client received live DATA: " + JSON.stringify(saw.slice(0, 60)),
    );
    rc.close();
  }
  ...
  await writeResumeRecord(
    ...
    scope,
    "999999",
    "/tmp/pi-bogus-resume-" + Date.now() + ".sock",
    "bogus",
  );
  await resumeBackgroundShells(sessDir, scope); // 2s connect timeout per record
  ok(
    !hookFired,
    "stale resume record cleaned up silently (no completion hook)",
  );
  ...
  ok(
    ...
    "resumed shell is listed in bg (manageable)",
  );
  ok(
    signalChild(rpid, "SIGINT") === true,
    "resumed shell is signalable via the resume socket",
  );
  killAll();
  ok(
    getShellBackgrounds().length === 0,
    "killAll removed the resumed shell from bg",
  );
