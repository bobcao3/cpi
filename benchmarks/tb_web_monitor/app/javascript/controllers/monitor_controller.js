...
export default class extends Controller {
  ...
  layout() {
    const h =
      this.element.querySelector("header")?.getBoundingClientRect().height ?? 0;
  ...
  selectTrial(event) {
    ...
    Turbo.visit(
      `/jobs/${encodeURIComponent(this.jobSelTarget.value)}/trial/${encodeURIComponent(tr.dataset.trial)}`,
      { frame: "detail" },
    );
  }
