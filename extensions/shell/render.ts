...
export function renderShCall(args: any, theme: any, context: any, defaultWaitfor: number, tailLines: number): Text {
  ...
  if (cmdLines.length <= tailLines) {
    ...
    const rest = cmdLines.slice(1).map((l: string) => theme.fg("toolTitle", l)).join("\n");
    text.setText(theme.fg("toolTitle", theme.bold(`$ ${cmdLines[0]}`)) + "\n" + rest + theme.fg("dim", " · Ctrl+O to collapse") + waitforSuffix);
  } else {
    const tailStart = cmdLines.length - 3;
    const hint = theme.fg("dim", ` showing ${tailStart}-${cmdLines.length} (Ctrl+O to expand)`);
    const tail = cmdLines.slice(-4).map((l: string) => theme.fg("toolTitle", theme.bold(l))).join("\n");
    text.setText(theme.fg("toolTitle", theme.bold(`$ ${cmdLines[0]}`)) + hint + waitforSuffix + "\n" + theme.fg("dim", "...") + "\n" + tail);
  }
  return text;
}

interface ShResultDetails { id?: string; exitCode?: number | null; status?: string; fullOutputPath?: string; describe?: string; shuckWarnings?: string; shuckBlocked?: boolean }

export function renderShResult(result: any, opts: { expanded: boolean; isPartial: boolean }, theme: any, tailLines: number): Text {
  ...
  const outputLines = outputText.trimEnd().split("\n").filter((l: string) => l !== "").map((t) =>
    t.replace(/\x1b\[[\x30-\x3f]*[\x20-\x2f]*[\x40-\x6c\x6e-\x7e]/g, "").replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, "").replace(/\x1b\_[^\x07]*(?:\x07|\x1b\\)/g, "").replace(/\x1bP[^\x1b]*(?:\x1b\\|\x9c)/g, "").replace(/\t/g, "   ").replace(/\r/g, ""),
  );
  ...
  if (isPartial) {
    ...
    let summary = theme.fg("warning", "⏳ running") + (hidden > 0 ? theme.fg("dim", ` · showing L${hidden + 1}-${outputLines.length} (Ctrl+O to expand)`) : theme.fg("dim", ` · ${outputLines.length} lines`));
    return outputText.trim()
      ? new Text(tail.map((l: string) => theme.fg("toolOutput", l)).join("\n") + "\n" + summary, 0, 0)
  ...
  else if (isRunning) { status = theme.fg("warning", "⏳ backgrounded"); if (details?.id) status += theme.fg("dim", ` PID=${details.id}`); if (details?.fullOutputPath) status += theme.fg("dim", ` · ${details.fullOutputPath}`); }
  ...
  if (expanded) {
    ...
    let t = totalLines > 0 ? outputLines.map((l: string) => theme.fg("toolOutput", l)).join("\n") : theme.fg("dim", "(no output)");
    if (details?.fullOutputPath) t += `\n${theme.fg("dim", `Full output: ${details.fullOutputPath}`)}`;
    return new Text(t + "\n" + summary, 0, 0);
  }

  if (totalLines === 0) return new Text(theme.fg("dim", "(no output)") + "\n" + status, 0, 0);
  if (totalLines <= tailLines) return truncatedPreview(outputLines.map((l: string) => theme.fg("toolOutput", l)), status + theme.fg("dim", ` · ${totalLines} lines`));
  const startLine = totalLines - tailLines + 1;
  let summary = status + theme.fg("dim", ` · showing L${startLine}-${totalLines} (Ctrl+O to expand)`);
  if (details?.fullOutputPath) summary += theme.fg("warning", " [truncated]");
  return truncatedPreview(outputLines.slice(-tailLines).map((l: string) => theme.fg("toolOutput", l)), summary);
}
